...
VkShaderModule VulkanPostPipeline::CreateShaderModule(VkDevice device, const std::vector<char>& code) {
...
void VulkanPostPipeline::Create(VkDevice device, VkExtent2D extent, VkRenderPass swapchainRenderPass) {

    std::string filename;

    filename = "fullscreen.vert.spv";
    auto vertCode = ReadFile("../Assets/Shaders/" + filename);
    std::cout << "[Shader] Loading: " << filename << std::endl;

    filename = "fullscreen.frag.spv";
    ...
    VK_CHECK(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &m_layout));
